package TransportationProject.algorithm;

import java.util.ArrayList;
import java.util.HashMap;

import TransportationProject.graph.Digraph;
import static TransportationProject.utils.Functions.*;
...
public class Dijkstra {
    ...
    private static void printDistances(Node destination) {
        System.out.println("Total Distance: " + String.format("%.3f", DISTANCE_MAP.get(destination) / 6F) + "km \n");
    }
    ...
    private static void printPrevious() {
        ...
        for (HashMap.Entry<Node, Node> entry : PREVIOUS_NODE.entrySet()) {
            ...
            if (PREVIOUS_NODE != null)
                System.out.println(node.getName() + " --> " + PREVIOUS_NODE.getName());
            else {
